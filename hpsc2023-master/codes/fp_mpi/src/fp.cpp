...
public:
  ...
  void ParticlesOnMesh(particles &PTCL, mpiInfo &myMPI)
  {
    ...
    for (int k = 1; k <= PTCL.n; ++k)
    {
      ...
      // "active" flag to zero (-1), and note the processor to which it is going for MPI exchange.

      // this fascilitates diagonal movement
      ...
      if (PTCL.active[k] == 1) 
      {
        ...
        if (PTCL.x[k] < x0) //left boundary on x-axis
        {
          PTCL.active[k] = -1;
          iPEnew = myMPI.iPE - 1;
        }
        if (PTCL.x[k] > x1) //right boundary on x-axis
        { /* TO-DO */
          PTCL.active[k] = -1;
          iPEnew = myMPI.iPE + 1;
        }
        if (PTCL.y[k] < y0) //bottom boundary on y-axis
        { /* TO-DO */
          PTCL.active[k] = -1;
          jPEnew = myMPI.jPE - 1;
        }
        if (PTCL.y[k] > y1) //top boundary on y-axis
        { /* TO-DO */
          PTCL.active[k] = -1;
          jPEnew = myMPI.jPE + 1;
        }
      ...
      if (PTCL.active[k] == -1)
      {
        if (iPEnew >= 0 && iPEnew < myMPI.nPEx) //nRealx left to right
          if (jPEnew >= 0 && jPEnew < myMPI.nPEy) //nRealy bottom to top
          {
...
int main(int argc, char *argv[])
{
  ...
  {
    if (!strcmp(argv[count], "-nPEx"))
      nPEx = atoi(argv[count + 1]);
    if (!strcmp(argv[count], "-nPEy"))
      nPEy = atoi(argv[count + 1]);
    if (!strcmp(argv[count], "-nCellx"))
      nCellx = atoi(argv[count + 1]);
    if (!strcmp(argv[count], "-nCelly"))
      nCelly = atoi(argv[count + 1]);
    if (!strcmp(argv[count], "-flux"))
      flux = atof(argv[count + 1]);
    if (!strcmp(argv[count], "-tEnd"))
      tEnd = atof(argv[count + 1]);
    if (!strcmp(argv[count], "-dt"))
      dt = atof(argv[count + 1]);
  }
  ...
  for (double t = 0.; t <= tEnd; t += dt)
  {
    ...
    if (myMPI.iPE == 0)
      PTCL.addFlux(t, MESH.y0, MESH.y1, density, vx_bdy, vy_bdy);
